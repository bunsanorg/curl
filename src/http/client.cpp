...
namespace bunsan{namespace curl{namespace http
{
    ...
    void client::set_method_get()
    {
        set(options::httpget(true));
    }

    void client::set_method_head()
    {
        set(options::nobody(true));
    }

    void client::set_method_post()
    {
        set(options::post(true));
    }

    void client::set_method_put()
    {
        set(options::upload(true));
    }
    ...
    void client::set_header(const header &h)
    {
        m_request_headers.merge_insert(h);
    }

    void client::set_header(const std::string &name,
                            const std::string &value)
    {
        set_header(header(name, value));
    }

    void client::set_header(const std::string &h)
    {
        set_header(header::parse(h));
    }
    ...
    void client::set_url(const std::string &url)
    {
        set(options::url(url));
    }
    ...
    const header_parser &client::responses()
    {
        return m_header_parser;
    }
    ...
    namespace
    {
        const not_ok_status_error &fill_status_error(
            const not_ok_status_error &e, const status &s)
        {
            if (s.version != http_version::http_none)
                e << not_ok_status_error::status_http_version(s.version);
            if (s.code)
                e << not_ok_status_error::status_code(s.code);
            if (!s.reason.empty())
                e << not_ok_status_error::status_reason(s.reason);
            return e;
        }
    }
    ...
    void client::check_perform()
    {
        ...
        if (s.code != 200)
            BOOST_THROW_EXCEPTION(
                fill_status_error(not_ok_status_error(), s));
    }
}}}
